...
* **UI更新 (v1.1.2 Update!)**  
設定したマスコン・ブレーキ段数やモード（電気指令式/自動空気ブレーキ）に合わせて、ウィンドウの縦幅が適切な大きさに変化するようになりました。  
* **マルチシミュレータ対応**  
JRETSに加え、BVE Trainsim、PCSX2 (PS2エミュレータ)、RPCS3 (PS3エミュレータ) 向けの専用変換ロジックを搭載。  
UIから対象のゲームをシームレスに切り替えることができます。  
* **ブレーキ段数を最大13段に拡張**   
コントローラー内部の接点チャタリングから、ブレーキ段数は**最大常用13段**の車両まで対応できるようになりました。
* **車両特化型の特殊モード**  
「各特殊モード」を有効にすることで、ゲーム内の特定の車両に合わせた操作を実現しています。
    * **抑速モード (New!)**: BVEやJRETSでブレーキ側に抑速ブレーキが1段搭載されている車両を運転する際、UI上のB1を「抑速」表示に、それ以降のブレーキ段数を-1段として表示し、実車のブレーキ段数と一致するようになりました。
...
* **高精度な入力制御と同期ロジック**
    * **完全なステート同期機能**: JRETS・BVEモードでは、ソフトウェア内部で現在のハンドル段数を厳密に記憶・管理し、目標段数との「差分」だけを正確にキー入力として送信します。これにより、素早いガチャガチャとした操作でも、物理コントローラーとゲーム内の段数がズレない安定性を実現しました。
    * **エミュレータの同期を最適化**: PCSX2・RPCS3等における「N（ニュートラル）に戻る際のアニメーション処理落ち」を考慮し、ハンドルを大きく跨ぐ操作時のみ、エミュレータ側がキー入力を取りこぼさないための専用の同期処理（0.05秒の最適化）を組み込んでいます。
    * **ノイズ除去フィルタ**: コントローラー特有の接点チャタリングや、移動中の一瞬の信号飛びを完全にカットします。
* **直感的なGUI**  
Pygameによる視覚的に分かりやすいインターフェースを目指しました。  
起動したまま、対象ゲームの変更、特殊モードのON/OFF、最大ノッチ数（最大5段）・ブレーキ段数（最大常用13段）の調整がマウス操作で完結します。
...
> [!NOTE]
> **※用語に関する注釈** > 本ツールではモードの区分として「電気指令式」「自動空気ブレーキ」という呼称を用いていますが、これらは**ゲーム内での挙動（応答性重視か、込め・重なり操作重視か）を区別するための便宜的な呼称**です。  
> 実車のブレーキシステムの厳密な定義や分類（電磁直通、電磁自動など）に準拠したものではありません。

> [!WARNING]
> **重要な仕様：勾配起動について** > 本ツールは入力の安定性を最優先するため、「ブレーキが入っている状態では、マスコン出力は強制的に『切 (N)』になる」仕様としています。  
...
#### 【注意事項】
※185系、キハ54系のハンドル操作について、「簡易」を選択した場合は「電気指令式モード」で運転してください。  
※185系、キハ54系のハンドル操作について、「リアル」を選択した場合は「自動空気ブレーキモード」で運転してください。  
※マスコン側に付いている抑速ブレーキには対応していません。   
※電気指令式モードの場合は、1ハンドル・2ハンドルの区別はありません。  
※運転する車両に応じて電気指令式モード・自動空気ブレーキモードを切り替え、適切なマスコン段数(最大5段)とブレーキ段数(最大13段)を設定してください。
...
| `SELECT` | Triangle(△)半押し | 通常警笛(※RPCS3モードでは京阪8000系モード時を除いて全押し) |
| `START` | START | START |
...
#### 【注意事項】 
・PCSX2、RPCS3共通  
※運転する車両に応じて1ハンドルモード・2ハンドルモードを切り替え、適切なマスコン段数(最大5段)とブレーキ段数(最大13段)を設定してください。
