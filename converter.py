...
# --- アイコン ---
try:
    # Python実行環境ではなく、独立したアプリとしてOSに認識させる
    myappid = 'my.dengo.converter.v3' 
    ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(myappid)
except:
    pass

# SDLの環境変数でバックグラウンドでの入力を許可する
...
def get_inputs():
    pygame.event.pump()
    if pygame.joystick.get_count() == 0: return 0, (0,0,0), [0]*17
...
def draw_bar_gauge(surface, center_x, start_y, current_val, max_val, is_mascon):
    ...
    for i in range(1, max_val + 1):
        ...
        is_active = (current_val >= i) if is_mascon else (current_val >= i and current_val <= max_val)
...
def draw_auto_brake_unit(surface, center_x, center_y, current_val):
    ...
    overlap_angle = 10 
    draw_solid_arc(surface, col_arc_run, center, r_inner, r_outer, 200, 270 + overlap_angle)
    draw_solid_arc(surface, col_arc_brk, center, r_inner, r_outer, 270 - overlap_angle, 380)

    angles = { 0: 200, 6: 270, 8: 330, 9: 20 }
    target_angle = angles.get(current_val, 200)
    labels = [(0, "運転", 200, COLOR_N), (6, "重なり", 270, COLOR_B_SVC), (8, "常用", 330, COLOR_B_SVC), (9, "非常", 20, COLOR_B_EMG)]
    ...
    brass_len, wood_len, grip_w, line_width = 35, 100, 28, 16
    arm_offset_perp = 0.6
    ox, oy = arm_offset_perp * math.sin(angle_rad), -arm_offset_perp * math.cos(angle_rad)
    overlap_len = brass_len + 30
    pygame.draw.line(surface, COLOR_HANDLE_BRASS, center, (center[0] + overlap_len * math.cos(angle_rad), center[1] + overlap_len * math.sin(angle_rad)), line_width)
    grip_center_dist = brass_len + wood_len / 2
    grip_center_x, grip_center_y = center[0] + grip_center_dist * math.cos(angle_rad) + ox, center[1] + grip_center_dist * math.sin(angle_rad) + oy
    grip_surf = pygame.Surface((wood_len, grip_w), pygame.SRCALPHA)
    pygame.draw.polygon(grip_surf, COLOR_HANDLE_WOOD, [(0, grip_w//2 - 10), (wood_len - 15, grip_w//2 - 14), (wood_len - 15, grip_w//2 + 14), (0, grip_w//2 + 10)])
    ...
    surface.blit(rotated_grip, rotated_grip.get_rect(center=(grip_center_x, grip_center_y)))
...
# --- グローバル設定変数 ---
current_mode = "1"
max_power, max_brake = 5, 8

def toggle_mode(): global current_mode; current_mode = "2" if current_mode == "1" else "1"
...
def main():
    ...
    init_p = MASCON_LEVEL_MAP.get(p_pat, 0)
    init_b = ELECTRIC_BRAKE_MAP.get(b_val, 0)
    mascon_filter, brake_filter = StableNotchReader(init_p), StableNotchReader(init_b)
    prev_p, prev_b, last_auto_s, p_start, p_select = init_p, init_b, 0, False, False

    mascon_center_x, elec_brake_center_x = MARGIN_SIDE + 50, SCREEN_WIDTH - MARGIN_SIDE - 50
    auto_brake_center_x, auto_brake_center_y = (SCREEN_WIDTH - MARGIN_SIDE) - 40 - (180 * math.cos(math.radians(20))), 250
    ...
    while running:
        ...
        raw_b = ELECTRIC_BRAKE_MAP.get(b_val, -1) if current_mode == "1" else AUTO_BRAKE_MAP.get(b_val, -1)
        raw_p = MASCON_LEVEL_MAP.get(p_pat, -1)
        filtered_b, filtered_p = brake_filter.update(raw_b), mascon_filter.update(raw_p)

        cur_p = min(filtered_p, max_power)
        if current_mode == "1":
            cur_b = max_brake + 1 if filtered_b == 9 else (0 if filtered_b == 0 else min(filtered_b, max_brake))
        else:
            cur_b = filtered_b
        ...
        lbl_mode_val = font_header_title.render("電気指令式" if current_mode == '1' else "自動空気ブレーキ", True, COLOR_ACCENT)
        screen.blit(lbl_mode_title, (180, header_h // 2 - lbl_mode_title.get_height()//2))
        screen.blit(lbl_mode_val, (180 + lbl_mode_title.get_width(), header_h // 2 - lbl_mode_val.get_height()//2))

        lbl_p_t = font_ui_label.render("マスコン段数", True, COLOR_TEXT_DIM)
        screen.blit(lbl_p_t, (mascon_center_x - lbl_p_t.get_width()//2, label_y))
        lbl_p_v = font_val.render(f"P{max_power}", True, COLOR_TEXT)
        screen.blit(lbl_p_v, (mascon_center_x - lbl_p_v.get_width()//2 - 10, val_y))

        if current_mode == "1":
            lbl_b_t = font_ui_label.render("ブレーキ段数", True, COLOR_TEXT_DIM)
            screen.blit(lbl_b_t, (elec_brake_center_x - lbl_b_t.get_width()//2, label_y))
            lbl_b_v = font_val.render(f"B{max_brake}", True, COLOR_TEXT)
            screen.blit(lbl_b_v, (elec_brake_center_x - lbl_b_v.get_width()//2 - 10, val_y))
        ...
        if current_mode == "1": draw_bar_gauge(screen, elec_brake_center_x, gauge_start_y, cur_b, max_brake, is_mascon=False)
        else: draw_auto_brake_unit(screen, int(auto_brake_center_x), int(auto_brake_center_y), cur_b)

        dbg_str = f"RAW: P={p_pat}  B=({(b_val >> 3) & 1}, {(b_val >> 2) & 1}, {(b_val >> 1) & 1}, {b_val & 1})"
        dbg = font_ui_label.render(dbg_str, True, (80, 80, 80))
        screen.blit(dbg, (20, 600 - 45))
        ...
        if current_mode == "2":
            ...
            if new_s != last_auto_s:
                ...
                elif new_s == 1 and last_auto_s == 0: pydirectinput.press(KEY_BRAKE_UP)
                elif new_s == 2: pydirectinput.keyDown(KEY_BRAKE_UP)
                elif new_s == 3: pydirectinput.keyDown(KEY_BRAKE_EMG)
                last_auto_s = new_s
        else:
            if cur_b != prev_b:
                if cur_b == max_brake + 1: pydirectinput.keyDown(KEY_BRAKE_EMG)
                else:
        ...
        if is_st != p_start: pydirectinput.keyDown(KEY_START) if is_st else pydirectinput.keyUp(KEY_START); p_start = is_st
        if is_sl != p_select: pydirectinput.keyDown(KEY_SELECT) if is_sl else pydirectinput.keyUp(KEY_SELECT); p_select = is_sl
